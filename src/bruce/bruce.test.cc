...
namespace {
  ...
  class TBruceTestServer final : public TFdManagedThread {
    ...
    TBruceTestServer(in_port_t broker_port, size_t msg_buffer_max_kb,
		     const std::string &bruce_conf, bool use_ssl = false)
    ...
    TBruceTestServer(in_port_t broker_port, size_t msg_buffer_max_kb,
		     std::string &&bruce_conf, bool use_ssl = false)
        : BrokerPort(broker_port),
          MsgBufferMaxKb(msg_buffer_max_kb),
  ...
  void CreateKafkaConfig(const char *topic, std::vector<std::string> &result,
    ...
    /* Add contents to a setup file for the mock Kafka server.
       ...
       starting at 10000 (or the port in the argument.  
       During the test, bruce will connect to these ports
       and forward messages it gets from the UNIX domain socket. */
    
    result.push_back("ports " + std::to_string(ports) + " 2");
